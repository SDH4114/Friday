...
export const docs: DocPage[] = [
  ...
  },
  {
    ...
    sections: [
      ...
      { id: "installer", title: "GitHub installer", blocks: [code("curl -fsSL https://raw.githubusercontent.com/SDH4114/Raya-APPLE/prime/install.sh | bash"), p("The installer checks the OS, installs Node 22 through nvm if necessary, clones the repository, runs npm ci and npm run build, packs the package, and installs the tarball globally. A genuinely fresh install initializes the default state. Existing state is preserved. If Raya is already installed, the public installer refuses direct replacement and asks you to use raya update so the checkpoint is not bypassed accidentally. Packaging avoids a global symlink into the temporary clone."), callout("note", "npm publication", "The source currently documents the npm package name @sdh4114/raya, but GitHub remains the working pre-publication install path.")] },
