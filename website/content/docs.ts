...
export const docs: DocPage[] = [
  ...
  },
  {
    ...
    sections: [
      ...
      { id: "installer", title: "GitHub installer", blocks: [code("curl -fsSL https://raw.githubusercontent.com/SDH4114/Raya-APPLE/prime/install.sh | bash"), p("The installer checks the OS, installs Node 22 through nvm if necessary, clones the repository, runs npm ci and npm run build, packs the package, and installs the tarball globally. Packaging avoids a global symlink into the temporary clone."), callout("note", "npm publication", "The source currently documents the npm package name @sdh4114/raya, but GitHub remains the working pre-publication install path.")] },
      { id: "development", title: "Development install", blocks: [code("git clone https://github.com/SDH4114/Raya-APPLE.git\ncd Raya-APPLE\nnpm install\nnpm run build\nnpm link\nraya") ] },
      { id: "update", title: "Update and uninstall", blocks: [p("There is no dedicated raya update or uninstall command in the current CLI. Re-running the GitHub installer installs the current prime build. Remove the global package with npm when you intentionally want to uninstall it."), code("npm uninstall -g @sdh4114/raya"), callout("warning", "Preserve your state", "Uninstalling the package does not remove ~/.raya. Back up or remove RAYA_HOME separately only when you intend to delete config, credentials, sessions, memory, and skills.")] },
  ...
  },
  {
    ...
    sections: [
      { id: "types", title: "Provider types", blocks: [table(["Type", "Examples", "Credential"], [["OAuth", "OpenAI Codex", "ChatGPT Plus / Pro / Codex OAuth"], ["API key", "Anthropic, OpenRouter, OpenCode Zen, Hugging Face", "Provider-specific key or token"], ["Local OpenAI-compatible", "Ollama, LM Studio, vLLM, llama.cpp", "Keyless by default"]])] },
      { id: "connect", title: "Connect and inspect", blocks: [code("raya login\nraya login anthropic\nraya providers\nraya models --provider anthropic\nraya status\nraya logout anthropic"), p("The interactive /providers menu can connect or update credentials and select a provider. /models lists models across configured providers.")] },
      { id: "model-settings", title: "Model settings", blocks: [p("Raya records provider, model, reasoning level, context window, and maximum output tokens. /thinking only shows levels reported as supported by the active model. Tool-calling quality and image support depend on the selected model and provider adapter."), callout("limitation", "Reasoning is not orchestration", "The selected model provides reasoning. Raya supplies system context, tools, skills, memory, sessions, approvals, and interfaces.")] }
    ]
  },
  {
    slug: "local-models",
  ...
  },
  {
    ...
    sections: [
      ...
      { id: "soul", title: "SOUL.md", blocks: [p("SOUL.md defines user-authored tone, style, and character. It is deliberately visible and editable—not a hidden system prompt.")] },
