...
export const docs: DocPage[] = [
  {
    ...
    sections: [
      ...
      { id: "install", title: "1. Install Raya", blocks: [p("Windows PowerShell:"), code("irm https://raw.githubusercontent.com/SDH4114/Raya-APPLE/prime/install.ps1 | iex", "powershell"), p("macOS or Linux:"), code("curl -fsSL https://raw.githubusercontent.com/SDH4114/Raya-APPLE/prime/install.sh | bash"), p("Each GitHub-source installer clones the prime branch into a temporary directory, builds a package tarball, installs it globally, syncs built-in skills, creates the default SOUL.md when it is missing, and removes the temporary checkout.")] },
  ...
  },
  {
    ...
    sections: [
      ...
      { id: "installer", title: "GitHub installer", blocks: [p("Windows PowerShell:"), code("irm https://raw.githubusercontent.com/SDH4114/Raya-APPLE/prime/install.ps1 | iex", "powershell"), p("macOS or Linux:"), code("curl -fsSL https://raw.githubusercontent.com/SDH4114/Raya-APPLE/prime/install.sh | bash"), p("The Windows installer uses existing Node.js 22+ and Git, or provisions missing prerequisites through winget. It builds Raya with Windows-safe scripts, installs the packed archive globally, persists npm's global command directory in the user PATH, and makes raya.cmd available immediately. The Unix installer provisions Node through nvm when necessary. Both installers preserve existing state, initialize only a genuinely fresh installation, and avoid a global link into the temporary checkout."), p("The normal update path creates a complete local checkpoint before selecting install.ps1 on Windows or install.sh on macOS/Linux. Both receive the exact checked Git commit and a disposable RAYA_HOME. Older clients get the same compatibility checkpoint before replacement."), callout("note", "npm publication", "The source currently documents the npm package name @sdh4114/raya, but GitHub remains the working pre-publication install path.")] },
