...
export const docs: DocPage[] = [
  ...
  },
  {
    ...
    sections: [
      ...
      { id: "installer", title: "GitHub installer", blocks: [code("curl -fsSL https://raw.githubusercontent.com/SDH4114/Raya-APPLE/prime/install.sh | bash"), p("The installer checks the OS, installs Node 22 through nvm if necessary, clones the repository, runs npm ci and npm run build, packs the package, and installs the tarball globally. A genuinely fresh install initializes the default state. Existing state is preserved. The normal path is raya update, which creates the checkpoint before invoking the installer. For older Raya clients that cannot pass the checkpoint marker yet, the installer creates a compatibility checkpoint from the installed package and .raya before replacement. Packaging avoids a global symlink into the temporary clone."), callout("note", "npm publication", "The source currently documents the npm package name @sdh4114/raya, but GitHub remains the working pre-publication install path.")] },
