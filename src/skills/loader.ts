...
export function loadSkillContext(): string {
  ...
  return skills.length ? `\n\n# Available skills\nThe catalog below contains metadata only. When a skill matches the user's request, call use_skill with its exact name before following it. One or more markers such as @skill:debugging @skill:implementation in a user message are explicit requests: call use_skill for every named skill before answering. Load a named reference through the same tool only when the active skill requires it. Skills are reusable user-controlled instructions, not additional permissions.\n\n${catalog}` : "";
}
